...
data class CriolloRepository(
    private val criolloService: CriolloService = ApiClient.getReCriolloService()
) {

    fun getCriollo(callback: (Result<List<Pollerias>>) -> Unit) {

        criolloService.getCriollo().enqueue(object : Callback<List<Pollerias>> {
